...
def ejecutar_modelo_ampl(mod_path, dat_path, solver='gurobi', max_nodes=None, time_limit=None):
    ...
    try:
        ...
        best_solution, best_fitness = iterated_local_search(
            cap_cost_array, demand_array, facilities, demand_total
        )
        
        print("\nSolución inicial generada por ILS:")
        print(f"Centros abiertos: {best_solution}")
        print(f"Costo inicial: {best_fitness}")
        
        # Configuramos los centros abiertos en AMPL
        ...
        gurobi_options = f'MIPGap=1e-4 TimeLimit={time_limit}'
        ...
        for i in D:
            for j in C:
                ...
                if asignacion > 0:
                    print(f"Cliente {i} asignado a Centro {j}: {asignacion}")
